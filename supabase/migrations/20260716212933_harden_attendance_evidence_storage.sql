-- Attendance evidence must never be publicly readable or anonymously writable.
-- Access is resolved from the authenticated app user and their organization/site scope.

create or replace function public.can_upload_attendance_evidence(p_object_name text)
returns boolean
language sql
stable
security definer
...
create or replace function public.can_read_attendance_evidence(p_object_name text)
returns boolean
language sql
stable
security definer
...
revoke all on function public.can_upload_attendance_evidence(text) from public, anon;
revoke all on function public.can_read_attendance_evidence(text) from public, anon;
grant execute on function public.can_upload_attendance_evidence(text) to authenticated;
grant execute on function public.can_read_attendance_evidence(text) to authenticated;

update storage.buckets
set public = false
where id in ('attendance-photos', 'evidencias-asistencia');
...
with check (
  ...
  and public.can_upload_attendance_evidence(name)
);
...
using (
  bucket_id in ('attendance-photos', 'evidencias-asistencia')
  and public.can_read_attendance_evidence(name)
);
