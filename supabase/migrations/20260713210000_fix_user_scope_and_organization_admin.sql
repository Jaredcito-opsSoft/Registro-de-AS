-- Corrige asignaciones heredadas y separa el administrador de organizacion
-- del supervisor/usuario vinculado a un sitio.
...
begin
  ...
  select * into v_target from public.usuarios_app where id = p_usuario_id limit 1;
  ...
  select * into v_site from public.sitios
  where id = p_sitio_id and coalesce(activo, true) limit 1;
...
begin
  ...
  select * into v_actor from public.usuarios_app
  where auth_user_id = auth.uid() and coalesce(activo, true)
  order by public.app_role_rank(rol) desc, updated_at desc nulls last limit 1;
  ...
  select * into v_organization from public.organizaciones
  where id = p_organizacion_id and coalesce(activo, true) limit 1;
  ...
  values (
    ...
    jsonb_build_object('usuario_id', v_target.id, 'organizacion_anterior', v_target.organizacion_id, 'rol_anterior', public.normalize_app_role(v_target.rol), 'rol_nuevo', 'admin')::text,
