...
-- Allow organization administrators to register attendance at an explicitly selected site.
-- Privileged roles bypass schedule windows but retain site, GPS, evidence and duplicate checks.

alter table public.usuarios_app drop constraint if exists usuarios_app_role_check;

drop function if exists public.registrar_entrada_segura(text, text, text, jsonb, boolean, jsonb, text, text, text, integer, integer, integer, timestamptz, text, text, boolean, boolean, text, numeric, numeric, numeric, text);
...
begin
  ...
  if v_role in ('admin', 'superadmin') then
    if p_sitio_id is null then raise exception 'sitio_requerido_para_admin'; end if;
    select s.* into v_site
    from public.sitios s
    where s.id = p_sitio_id and s.activo = true;

    if v_site.id is null then raise exception 'sitio_activo_no_encontrado'; end if;
    if v_role = 'admin'
       and (v_user.organizacion_id is null or v_site.organizacion_id is distinct from v_user.organizacion_id) then
      raise exception 'sitio_fuera_de_alcance';
    end if;
  else
    if v_user.sitio_id is null then raise exception 'usuario_sin_sitio_asignado'; end if;
    if p_sitio_id is not null and p_sitio_id is distinct from v_user.sitio_id then
      raise exception 'sitio_fuera_de_alcance';
    end if;
    select s.* into v_site
    from public.sitios s
  ...
  insert into public.asistencias (
    ...
    v_site.organizacion_id, v_user.id, v_site.id, v_site.nombre, v_site.radio_metros,
  ...
  values (case when v_role in ('admin', 'superadmin') then 'asistencia_entrada_admin_sitio' else 'asistencia_entrada_sitio' end, v_user.id::text || ' / ' || v_site.id::text, case when v_geo_complete then 'ok' else 'revision' end, p_foto_entrada_user_agent);
  return v_row;
end;
$$;
...
begin
  ...
  where a.usuario_id = v_user.id and a.hora_salida is null
  order by a.hora_entrada desc limit 1 for update;
  if v_record.id is null then
    if exists (select 1 from public.asistencias a where a.usuario_id = v_user.id and a.fecha = (v_now at time zone 'America/Mexico_City')::date) then
  ...
  where s.id = coalesce(v_record.sitio_entrada_id, v_record.sitio_id)
    and s.organizacion_id = v_record.organizacion_id
    and s.activo = true;
...
revoke all on function public.registrar_entrada_segura(text, text, text, jsonb, boolean, jsonb, text, text, text, integer, integer, integer, timestamptz, text, text, boolean, boolean, text, numeric, numeric, numeric, text, uuid) from public, anon;
...
grant execute on function public.registrar_entrada_segura(text, text, text, jsonb, boolean, jsonb, text, text, text, integer, integer, integer, timestamptz, text, text, boolean, boolean, text, numeric, numeric, numeric, text, uuid) to authenticated;
